...
if(isset($_REQUEST["id_grupo_acesso"])){

    $loIdGrupoAcesso = $_REQUEST["id_grupo_acesso"];
    $loAcao = "U";
    $loDados = NULL;

    $loDados = array( "id_grupo_acesso" => $loIdGrupoAcesso, "status" );
    $loGrupoAcessoBO = new grupoAcessoBO();
    $loListaProduto =  $loGrupoAcessoBO->ConsultaGrupo($loDados);

    foreach ($loListaProduto as $row) {

        $loIdCodigoGrupo    = $row->mbCodigoGrupo;     
        $loNome             = $row->mbNome;          
        $loDataCad          = $row->mbDataCad;         
        $loDataAlt          = $row->mbDataAlt;         
        $loNomePessoaCad    = $row->mbPessoaCad; 
        $loNomePessoaAlt    = $row->mbPessoaAlt; 
        $loStatus           = $row->mbStatus;

    }

}
if($loIdCodigoGrupo == 3){
    $loADM = true;
}else{
    $loADM = false;
}
                            ...
                            <div class="portlet light bordered">
                                <div class="portlet-title">
                                    <div class="caption">
                                        ...
                                        <span class="caption-subject font-dark sbold uppercase"> Adiciona Grupo Acesso </span>
                                    </div>
                                               ...
                                               <div class="form-group">
                                                ...
                                                <div class="col-md-2">
                                                    <select class="form-control" id="status">
                                                        ...
                                                        <option <?php if($loStatus == 0  ){ echo "selected"; }?> value="0" >Desativado</option>
                                                    </select>                                                    
                                                </div>
                                        ...
                                        <div class="form-actions">
                                            <div class="row">
                                                <div class="col-md-offset-3 col-md-9">
                                                    <button type="button" class="btn dark" id="btn-adicionar" <?php if($loADM) { echo "disabled"; } ?>  >Adicionar</button>
                                                    <button type="button" id="btn-cancelar" class="btn default">Cancelar</button>
                                                    <input type="hidden" id="acao" value="<?php echo $loAcao; ?>" /> 
                                                    <input type="hidden" id="id_grupo_acesso" value="<?php echo $loIdCodigoGrupo; ?>" />
                                                </div>
                                            </div>
                                        </div>
                                    </form>
